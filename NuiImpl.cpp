...
DWORD WINAPI CSkeletalViewerApp::Nui_ProcessThread()
{
    ...
    while ( continueProcessing )
    {
		...
		/*ofstream outFile;
		outFile.open("SKEL.txt", ios::out | ios::app);
		if (outFile.is_open() ) {
			outFile << "T " << TIME << "\n";
		}
...
void CSkeletalViewerApp::Nui_BlankSkeletonScreen (HWND hWnd, bool getDC )
{
	...
	//HERE WE CAN PUT SOMETHING IF THERE ARE NO SKELETONS
}
...
void CSkeletalViewerApp::Nui_DrawSkeleton( NUI_SKELETON_DATA * pSkel, HWND hWnd, int WhichSkeletonColor )
{
	...
	double rad = atan2(pSkel->SkeletonPositions[0].x,pSkel->SkeletonPositions[0].z);
	double angle = rad*180/3.14159;
	ss << pSkel->SkeletonPositions[0].x << " " << pSkel->SkeletonPositions[0].y << " " << pSkel->SkeletonPositions[0].z << "\n";
	
	PostMessageW( m_hWnd, WM_UPDATE_ANGLE, IDC_ANGLE, angle );
    ...
    for (i = 0; i < NUI_SKELETON_POSITION_COUNT; i++)
    {
		...
		//ss << abs(pSkel->SkeletonPositions[0].x-pSkel->SkeletonPositions[i].x) << " " << abs(pSkel->SkeletonPositions[0].y-pSkel->SkeletonPositions[i].y) << " " << abs(pSkel->SkeletonPositions[0].z-pSkel->SkeletonPositions[i].z) << "\n";
		double xRot = cos(rad)*(pSkel->SkeletonPositions[i].x-pSkel->SkeletonPositions[0].x) - sin(rad)*(pSkel->SkeletonPositions[i].z-pSkel->SkeletonPositions[0].z) + pSkel->SkeletonPositions[0].x;
		double zRot = sin(rad)*(pSkel->SkeletonPositions[i].x-pSkel->SkeletonPositions[0].x) + cos(rad)*(pSkel->SkeletonPositions[i].z-pSkel->SkeletonPositions[0].z) + pSkel->SkeletonPositions[0].z;

		ss << pSkel->SkeletonPositions[0].x-xRot << " " << pSkel->SkeletonPositions[0].y - pSkel->SkeletonPositions[i].y << " " << pSkel->SkeletonPositions[0].z-zRot << " ";
		
	}

	//ss << pSkel->SkeletonPositions[0].x << " " << pSkel->SkeletonPositions[0].z << " " << atan2(pSkel->SkeletonPositions[0].x,pSkel->SkeletonPositions[0].z)*180/3.14159 <<"\n";

	/*double xRot = cos(rad)*(pSkel->SkeletonPositions[2].x-pSkel->SkeletonPositions[0].x) - sin(rad)*(pSkel->SkeletonPositions[2].z-pSkel->SkeletonPositions[0].z) + pSkel->SkeletonPositions[0].x;
	double zRot = sin(rad)*(pSkel->SkeletonPositions[2].x-pSkel->SkeletonPositions[0].x) + cos(rad)*(pSkel->SkeletonPositions[2].z-pSkel->SkeletonPositions[0].z) + pSkel->SkeletonPositions[0].z;

	ss << pSkel->SkeletonPositions[2].x << " " << xRot  << " " << pSkel->SkeletonPositions[2].z << " " << zRot << "\n";

	ss << abs(pSkel->SkeletonPositions[0].x-pSkel->SkeletonPositions[2].x) << " " << abs(pSkel->SkeletonPositions[0].x-xRot) << " ";
	ss << abs(pSkel->SkeletonPositions[0].z-pSkel->SkeletonPositions[2].z) << " " << abs(pSkel->SkeletonPositions[0].z-zRot) << "\n";*/
	...
	outFile.open("SKEL.txt", ios::out | ios::app);
	if (outFile.is_open() )
		outFile << "T " << TIME << " " << pSkel->dwTrackingID << "\n";
		outFile << ss.str();
...
void CSkeletalViewerApp::Nui_GotSkeletonAlert( )
{
    ...
    if ( SUCCEEDED(m_pNuiSensor->NuiSkeletonGetNextFrame( 0, &SkeletonFrame )) )
    {
        for ( int i = 0 ; i < NUI_SKELETON_COUNT ; i++ )
        {
                ...
                (SkeletonFrame.SkeletonData[i].eTrackingState == NUI_SKELETON_POSITION_ONLY && m_bAppTracking))
            {
				...
				//store the tracked skeleton ids
				if (currSkel[0] == -1) 
					currSkel[0] = SkeletonFrame.SkeletonData[i].dwTrackingID;
				else 
					currSkel[1] = SkeletonFrame.SkeletonData[i].dwTrackingID;

			}
			
			if ( SkeletonFrame.SkeletonData[i].eTrackingState == NUI_SKELETON_POSITION_ONLY || SkeletonFrame.SkeletonData[i].eTrackingState == NUI_SKELETON_TRACKED) {
				numSkels++;

				bool repeatSkel = false;

				//store skel if its not in the ordered vector already
				for (vector<int>::iterator it = orderSkels.begin(); it != orderSkels.end(); it++)
					if (*it == SkeletonFrame.SkeletonData[i].dwTrackingID) {
						repeatSkel = true;
						break;
					}

				if (!repeatSkel) orderSkels.push_back(SkeletonFrame.SkeletonData[i].dwTrackingID);
	...
	//need to delete skels that are no longer present!!
	for (vector<int>::iterator it = orderSkels.begin(); it != orderSkels.end();) {
		bool found = false;
	...
	if (numSkels > 2) {
		...
		Nui_SetApplicationTracking(true);
		int temp[2] = {-1, -1};

		//find the tracked skeletons in the ordered vector
		for (int i = 0; i < numSkels; i++)
			if (orderSkels[i] == currSkel[0]) //once found, track the next in the ordered vector
				temp[0] = orderSkels[(i+1)%numSkels];
			else
				if (orderSkels[i] == currSkel[1])
					temp[1] = orderSkels[(i+1)%numSkels];
		
		Nui_SetTrackedSkeletons(temp[0], temp[1]);

	}
	else Nui_SetApplicationTracking(false);
...
void CSkeletalViewerApp::Nui_SetApplicationTracking(bool applicationTracks)
{
    if ( HasSkeletalEngine(m_pNuiSensor) )
    {
		...
		//HRESULT hr = m_pNuiSensor->NuiSkeletonTrackingEnable( m_hNextSkeletonEvent, NUI_SKELETON_TRACKING_FLAG_TITLE_SETS_TRACKED_SKELETONS);
        if ( FAILED( hr ) )
        {
            MessageBoxResource(IDS_ERROR_SKELETONTRACKING, MB_OK | MB_ICONHAND);
        }
